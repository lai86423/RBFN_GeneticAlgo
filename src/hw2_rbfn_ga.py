...
def initWeight(dim_i, group, node_j):
    ...
    weight2 = np.random.rand(group, node_j)     # sigma(σ) 值域0~1(Gussian)
...
#----------------------------------------------------------------------------
# 實數型基因演算法
def geneAlgo(fitFuncO, mutationP, crossoverP, weight, node, group, minIndex): #實數型基因演算法 
    #-------------------------------------
...
def SaveModelParams(weight,node,group,dim):
    theta = weight[0]
    w = weight[1:1 + node]
...
def Main():
    ...
    group = int(var_group.get())
    node = int(var_node.get())
    mutationP = float(var_mutationP.get())
    crossoverP = float(var_crossoverP.get())
    optin_saveM = var_option.get() #0/1

    #initWeight 
    weight = initWeight(dim - 1, group, node)   #x_i => dim - 1：扣掉最後一項預期輸出
    y = []
...
if __name__ == "__main__":
    ...
    window.title('HW2 - RBFN x Genetic Algo')
    ...
    label_group = tk.Label(frame1, text='Group = ')
    label_group.pack(side=tk.LEFT, padx=(10,0))

    var_group=tk.StringVar()
    ent_group = tk.Entry(frame1, textvariable = var_group, width=4)
    ent_group.pack(side=tk.LEFT)
    ...
    label_mutationP = tk.Label(frame2, text='Mutation Rate = ')
    label_mutationP.pack(side=tk.LEFT, padx=(10,0))

    var_mutationP=tk.StringVar()
    ent_mutationP = tk.Entry(frame2, textvariable=var_mutationP, width=5)
    ent_mutationP.pack(side=tk.LEFT, pady=5)

    label_crossoverP = tk.Label(frame2, text='Crossover Rate = ')
    label_crossoverP.pack(side=tk.LEFT, padx=(10,0))

    var_crossoverP=tk.StringVar()
    ent_crossoverP = tk.Entry(frame2, textvariable = var_crossoverP, width=5)
    ent_crossoverP.pack(side=tk.LEFT, pady=5)
